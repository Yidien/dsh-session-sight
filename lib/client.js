...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 把一条斜杠命令行发给当前会话执行；失败上屏提示（升级后签名不匹配也不会静默失效）。 */
		function execute(ctx, line, { refreshAfter = false, onSettled = null } = {}) {
			try {
				...
				const settle = async () => {
					// 删除/彻底删除成功后，就地刷新侧栏会话列表（官方姿势），
					// 否则被删会话会以「未分组」残影继续显示。
					if (refreshAfter) {
						try {
							const sessions = ctx.get("sessions");
							// 官方运行时里会话服务暴露的是 refresh()（内部走 manager.refreshList()），
							// 老版本 shell 可能是 refreshList()，两个都探测。
				...
				Promise.resolve(run.call(commands, current, line, []))
					.then((result) => {
						// 升级后的 RPC 返回 {ok, value}，命令本身的结果在 value.result。
		...
		function openPicker(ctx, entry) {
			...
			/**
			 * 执行一行动作；目标是当前会话时，先跳到非当前的第一条会话，再对原会话归档/删除。
			 * 命令通道会随之改用新会话的 agent，host 端 refuseLive 才放行（只拦真正 running 的）。
			 */
			function runRowAction(ctx, row, act, all, reload) {
				// 彻底删除（danger）动作：记录 id，面板列表即时隐藏该会话（host 侧无逐出 API，重启前会残留）。
				if (act.danger === true) {
					const n = String(row.id ?? "");
					forgottenIds.add(n.startsWith("session-") ? n.slice("session-".length) : n);
				}
				if (row.current !== true) {
					execute(ctx, `${act.command} ${row.id}`, { refreshAfter: act.refreshAfter === true, onSettled: reload });
					return;
				}
				...
				// 等快照 current 变为目标会话再执行（命令通道会用新 agent）。
					...
					react.createElement("div", { key: "acts", style: { flexShrink: 0, display: "flex", gap: "6px" } },
						entry.rowActions.map((act) => {
							...
							return react.createElement("button", {
								...
								type: "button",
								disabled,
								title: isCurrent && !isPreview ? "当前会话：将先切换到其他会话再操作" : act.label,
								onClick: () => {
									if (isPreview) {
