...
async function rebuildRegistryIndex(ctx) {
	try {
		...
		await registry.replaceHeaderIndex((await persistence.list()).map((snapshot) => snapshot.header));
		...
		for (const project of projects) {
			...
			for (const variant of variants) {
				try {
					...
					if (sessionLogEntry(entries) !== void 0) return true;
				} catch { /* 无此目录，继续 */ }
			}
		}
	} catch { /* sessions 根缺失 */ }
...
function sessionMetaFromProjcache() {
	...
	const setMeta = (id, title, lastPromptAt, createdAt) => {
		// 双 id 拼写都登记，调用方按任一拼写查都能命中
	...
	// 0.1.2 起 per-record 布局：storages/session_projcache/sessions/<id>.json（version 5）
	try {
		...
		for (const file of readdirSync(dir)) {
			...
			try {
				...
				const rec = doc?.record ?? doc; // 兼容 { record: {...} } 与裸 { identity, rows }
				const rows = rec?.rows ?? {};
				setMeta(file.slice(0, -".json".length), rows?.title?.val, rows?.sessionListMetadata?.val?.lastPromptAt, rec?.identity?.createdAt);
			} catch {
				/* 单文件损坏：跳过，继续其它 */
			}
		}
	...
	// 旧单文件回退（v3，0.1.1-rc.2 及更早），仅补 new 目录里没有的条目
	try {
		...
		for (const [id, row] of Object.entries(j?.tables?.sessions ?? {})) {
			if (!out.has(String(id))) {
				setMeta(String(id), row?.rows?.title?.val, row?.rows?.sessionListMetadata?.val?.lastPromptAt, row?.identity?.createdAt);
			}
		}
	} catch {
...
/**
* 会话目录里的日志文件：兼容旧命名 `session.jsonl(.zstd)` 与新版 `session.v<N>.jsonl(.zstd)`。
* 同名多代并存时取版本号最高的一代（与官方 resolveGenerationInDirectory 的取向一致）。
* @param entries - 会话目录下的文件名数组。
* @returns 选中的日志文件名；没有则 undefined。
*/
function sessionLogEntry(entries) {
	const versionOf = (name) => {
		const matched = /^session\.v(\d+)\./.exec(name);
		return matched === null ? 0 : Number(matched[1]);
	};
	const logs = entries.filter((name) => /^session(\.v\d+)?\.jsonl(\.zstd)?$/.test(name));
	if (logs.length === 0) return void 0;
	return logs.sort((left, right) => versionOf(right) - versionOf(left))[0];
}
/** 递归找 `<root>/<project>/<id>/session*`（深度 2，容错兜底）。 */
async function scanForLog(root, id) {
	...
	for (const project of projects) {
		...
		try {
			...
			const log = sessionLogEntry(entries);
			if (log !== void 0) return join(sessionDir, log);
		} catch {
			/* 无此会话目录，继续 */
		}
	}
...
async function locateSession(ctx, id) {
	...
	try {
		...
		const snapshot = snapshots.find((entry) => String(entry?.header?.id) === id);
		if (snapshot === void 0) return null;
		const loc = persistence.locate?.({ id, cwd: snapshot.header.cwd });
		logPath = loc?.path;
	} catch {
		logPath = void 0;
	}
...
*/
async function buildArchivedRows(ctx, archivedIds, titles) {
	const registry = getWorkspaceRegistry(ctx);
	...
	try {
		for (const snapshot of (await ctx.get("sessionPersistence")?.list?.()) ?? []) {
			const header = snapshot?.header;
			if (typeof header?.id === "string") {
				knownIds.add(String(header.id));
				if (header.createdAt !== void 0) createdAtOf.set(String(header.id), header.createdAt);
			}
		}
	} catch {
...
/** 全局 AgentHandle 生命周期跟踪（apply 时安装；删除已加载但空闲的会话时安全卸载）。 */
let agentTracker = null;

/** 待审批是 agent 仍存活时的独立状态，不能只依赖 agent.status。 */
function hasPendingApproval(ctx, id) {
	try {
		...
		for (const event of events) {
			if (event?.type === "approval/asked" && event?.data?.id !== void 0) open.add(event.data.id);
			else if (event?.type === "approval/decided" && event?.data?.id !== void 0) open.delete(event.data.id);
		}
		return open.size > 0;
	} catch {
		return false;
...
/** 运行或等待审批的目标绝不允许归档/删除。 */
function guardSessionInactive(ctx, id, verb) {
	if (hasPendingApproval(ctx, id)) return `会话 ${id} 正在等待审批，不能${verb}。请先在主界面完成审批。`;
	const agent = ctx.get("agents")?.get?.(id);
	if (agent?.status === "running") return `会话 ${id} 正在运行，不能${verb}。请等待任务结束后再操作。`;
	return null;
}

/**
 * 删除前守卫：运行/待审批一律拒绝；仅对已加载但空闲的目标调用 AgentHandle.dispose()
 * 以清理 live store，避免删除后出现“未分组”残影。
 */
async function guardDeleteReady(ctx, _invocation, id) {
	const blocked = guardSessionInactive(ctx, id, "删除");
	if (blocked !== null) return blocked;
	if (!agentTracker) {
		const agent = ctx.get("agents")?.get?.(id);
		if (agent !== void 0 || ctx.get("sessions")?.get(id) !== void 0) {
			return `会话 ${id} 已加载但无法安全卸载，未删除。`;
		}
		return null;
	}
	const result = await agentTracker.stopSession(id);
	return result.ok ? null : result.text;
}
/** 把会话目录移入回收站（保留 <项目目录>/<会话id> 结构，便于原路恢复）。 */
async function trashSession(ctx, invocation, id, titles) {
	const release = agentTracker ? agentTracker.reserve(id) : () => {};
	try {
		const guard = await guardDeleteReady(ctx, invocation, id);
		if (guard !== null) return { kind: "error", text: guard };
...
async function archivedRowsForRoute(ctx, titles) {
	const archivedIds = readArchivedIds(ctx);
	if (archivedIds === void 0) return null;
	if (archivedIds.length === 0) return [];
	return await buildArchivedRows(ctx, archivedIds, titles);
}
	...
	ctx.effect(() => webServer.register({
		...
		handler: async (_req, res) => {
			try {
				...
				const archived = await archivedRowsForRoute(ctx, titles);
				sendJson(res, 200, {
					trash,
					archived: archived ?? [],
...
function messageTextOf(event) {
	...
	// 0.1.2 起 assistant/message 正文移到 data.message.content（data.content 不再承载）
	let content = event?.data?.content;
	if (content === undefined && type === "assistant/message") content = event?.data?.message?.content;
...
/** 延迟绑定、需要复查的服务名：未到最终确认前这些缺失不报（避免 apply 时点误报）。 */
const SELFCHECK_WATCH = ["workspaceRegistry", "sessionPersistence", "storageDomain", "webServer", "httpServer"];
/** 是否已到最终确认时点（boot-settle，约 8s 后）。 */
let selfCheckSettled = false;
/** 最近一次自检行（去重：服务陆续就绪后复查不刷屏）。 */
let lastSelfCheckLine = "";

/**
 * 精简能力自检：只返回缺失项；全部可用返回空数组。
 * settled=false（apply/服务就绪复查）时，晚绑定服务缺失静默；
 * settled=true（boot-settle 最终确认）时缺失一律上报。
 */
function selfCheckMisses(ctx, settled) {
	const registry = getWorkspaceRegistry(ctx);
	const proj = ctx.get("storageDomain")?.get?.("session_projcache");
	const webServer = ctx.get(WEB_SERVER_KEYS[0]) ?? ctx.get(WEB_SERVER_KEYS[1]);
	const persistence = ctx.get("sessionPersistence");
	const miss = [];
	const add = (ok, name, hint) => {
		if (ok || (!settled && SELFCHECK_WATCH.includes(name))) return;
		miss.push(`${name}：${hint}`);
	};
	add(!!registry && typeof registry.archiveSession === "function", "workspaceRegistry", "缺少归档入口");
	add(!!registry && Array.isArray(registry.archivedSessionIds) && typeof registry.setState === "function", "workspaceRegistry", "archivedSessionIds/setState 缺失，归档列表与取消归档不可用");
	add(!!registry && typeof registry.list === "function" && typeof registry.replaceHeaderIndex === "function" && !!proj && typeof proj.table === "function", "workspaceRegistry", "workspace 账目 / projcache / 索引重建能力缺失，删除清账不完整");
	add(!!persistence && typeof persistence.list === "function" && typeof persistence.locate === "function", "sessionPersistence", "会话定位/预览不可用");
	add(!!webServer && typeof webServer.register === "function", "webServer", "面板列表/预览不可用");
	add(agentTracker !== null, "AgentHandle 跟踪", "删已加载会话有内存残影（重启后消失）");
	return miss;
}

/** 打印精简自检；未到最终确认前不做“全部就绪”断言（只有确定有问题才提前报）。 */
function reportSelfCheck(ctx, reason) {
	const miss = selfCheckMisses(ctx, selfCheckSettled);
	if (miss.length === 0 && !selfCheckSettled) return;
	const text = miss.length === 0 ? "全部就绪。" : `降级 ${miss.length} 项：\n  !! ${miss.join("\n  !! ")}`;
	const line = `[session-tools] 能力自检（${reason}）：${text}`;
	if (line === lastSelfCheckLine) return;
	lastSelfCheckLine = line;
	console.info(line);
}

function apply(ctx) {
	// AgentHandle 生命周期跟踪：删除已加载会话时先安全停止并从 live store 移除（免重启清残影）。
	agentTracker = createAgentTracker(ctx.get("agents"), ctx.get("sessions"));
	ctx.effect(() => () => {
		if (agentTracker) { try { agentTracker.release(); } catch { /* 忽略 */ } }
		agentTracker = null;
	}, "session-tools: agent tracker");
	...
	register({
		...
		handler: async (invocation) => {
			...
			const titles = titlesFromProjcache();
			const ids = new Set((await ctx.get("sessionPersistence").list()).map((snapshot) => String(snapshot.header.id)));
	...
	register({
		...
		handler: async (invocation) => {
			...
			if (Array.isArray(resolved)) return { kind: "error", text: `"${arg}" 匹配到多个会话，请指定 id：\n${candidateText(resolved, titles)}` };
			const release = agentTracker ? agentTracker.reserve(resolved) : () => {};
			try {
				const guard = await guardDeleteReady(ctx, invocation, resolved);
				if (guard !== null) return { kind: "error", text: guard };
	...
	register({
		...
		handler: async () => {
			...
			const titles = titlesFromProjcache();
			const rows = await buildArchivedRows(ctx, archivedIds, titles);
			return {
				kind: "success",
	...
	register({
		...
		handler: async (invocation) => {
			...
			if (Array.isArray(resolved)) return { kind: "error", text: `"${arg}" 匹配到多个会话，请指定 id：\n${candidateText(resolved, titles)}` };
			try {
				const guard = guardSessionInactive(ctx, resolved, "归档");
				if (guard !== null) return { kind: "error", text: guard };
	...
	ctx.on("internal/service", (serviceName) => {
		...
		if (SELFCHECK_WATCH.includes(serviceName)) reportSelfCheck(ctx, `服务 ${serviceName} 就绪`);
	});
	reportSelfCheck(ctx, "apply");
	// 兜底确认：boot 全部就绪后（延迟绑定服务可能晚于 apply）补一次最终报告。
	ctx.effect(() => {
		const t = setTimeout(() => {
			selfCheckSettled = true;
			reportSelfCheck(ctx, "boot-settle");
		}, 8000);
		return () => clearTimeout(t);
	}, "session-tools: 能力自检延时确认");
}
//#endregion
