...
/** 全局 AgentHandle 生命周期跟踪（apply 时安装；删除已加载会话时先用它安全停止）。 */
let agentTracker = null;
/** 删除前守卫的“运行中二次确认”武装窗口（毫秒）。 */
const PENDING_CONFIRM_MS = 30_000;
/** 运行中会话的二次确认记录：id → 首次提示时间。30 秒内再次执行同一命令视为确认。 */
const pendingConfirm = new Map();
/**
 * 删除前守卫（旧 refuseLive 的升级版）：
 *  - 「以当前会话的 agent 身份删自己」（命令行直删当前会话）仍拒绝；
 *  - 目标正处于运行中：第一次提示“会中断当前任务”，30 秒内再次执行同一命令才确认；
 *  - 已加载会话确认后先走 AgentHandle.dispose() 安全停止并从 live store 移除（免重启清残影）；
 *  - tracker 未就绪/停止失败 → 拒绝并给出原因。
 * @returns null（可删）或错误文本。
 */
async function guardDeleteReady(ctx, invocation, id) {
	if (invocation.agent?.session?.id === id) return "不能删除当前正在使用的会话。";
	if (!agentTracker) {
		// 回退旧语义：agent 真实 running 才拒；agents 不可用且 resident 即视为运行中。
		const agent = ctx.get("agents")?.get?.(id);
		if (agent !== void 0) {
			if (agent.status === "running") return `会话 ${id} 正在运行，不能删除。`;
		} else if (ctx.get("sessions")?.get(id) !== void 0) {
			return `会话 ${id} 正在运行，不能删除。`;
		}
		return null;
	}
	const agent = ctx.get("agents")?.get?.(id);
	if (agent !== void 0 && agent.status === "running") {
		const armedAt = pendingConfirm.get(id);
		if (armedAt !== void 0 && Date.now() - armedAt < PENDING_CONFIRM_MS) {
			pendingConfirm.delete(id); // 30 秒内第二次执行 = 用户确认
		} else {
			pendingConfirm.set(id, Date.now());
			return `会话 ${id} 正在运行，删除会中断其当前任务。已为你提示一次：请再执行一次同一命令以确认删除（${PENDING_CONFIRM_MS / 1000} 秒内有效；确认后会先安全停止再删除）。`;
		}
	} else {
